...
const CHART_CONFIG: ChartConfigItem[] = [
  {
    ...
    color: "var(--chart-1)",
    type: "bar",
    fallback: "Categorization data is currently being built. Most companies are tagged under key sectors like Tech Giant and Product.",
  },
  {
    key: "byEmployeeSize",
    ...
    color: "var(--chart-3)",
    type: "bar",
    fallback: "Scale data is pending. Early indicators show a mix of high-growth startups and established enterprises.",
  },
  {
    ...
    color: "var(--chart-2)",
    type: "bar_horizontal",
    fallback: "Hiring velocity data is currently limited. Based on available data, most companies fall under moderate patterns.",
  },
  {
    key: "byProfitability",
    ...
    color: "var(--chart-4)",
    type: "pie",
    fallback: "Financial stability metrics are being aggregated for a more accurate profitability mix.",
  },
  {
    key: "byRemotePolicy",
      ...
      {visibleCharts.length < CHART_CONFIG.length && (
        <div className="mt-8 rounded-xl border border-dashed border-border p-8 text-center bg-secondary/20">
          ...
          <p className="mt-1 text-xs text-muted-foreground max-w-sm mx-auto text-balance">
            ...
            <Link to="/categories" className="text-accent hover:underline">Categories</Link> or{" "}
            <Link to="/skill-mapping" className="text-accent hover:underline">Skill Mapping</Link> for detailed preparedness insights.
          </p>
        </div>
      )}
    </div>
...
function InsightFallback({ message }: { message: string }) {
  return (
    <div className="flex flex-col items-center justify-center h-[300px] text-center p-6 bg-secondary/10 rounded-lg border border-dashed border-border">
      ...
      <p className="text-sm font-medium text-foreground max-w-xs text-balance">
        {message}
      </p>
