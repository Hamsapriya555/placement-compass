...
const searchSchema = z.object({
  ...
  employee_size: fallback(z.string().optional(), undefined),
  ...
  sort: fallback(
    z.enum(["name", "employee_size", "yoy_growth_rate", "brand_value"]),
    "name",
  ).default("name"),
...
function ExplorePage() {
  ...
  const list = useCompanies({
    ...
    employeeSize: search.employee_size ?? null,
  ...
  return (
    <div className="mx-auto max-w-screen-2xl px-4 py-8 sm:px-6">
      ...
      <div className="mt-6 grid grid-cols-1 gap-3 rounded-xl border border-border bg-surface p-4 md:grid-cols-12">
        ...
        <FacetSelect
          label="Employee size"
          value={search.employee_size}
          options={facets.data?.employee_size}
          onChange={(v) => setSelect("employee_size", v)}
        />
        <FacetSelect
          label="Profitability"
