  ...
  const filtered = useMemo(() => {
    ...
    return allEntries.filter((item) => {
      ...
      const matchesRoleCategory =
        roleCategory === ANY ||
        item.roles.some((r) => r.role_category === roleCategory);

      const matchesOpportunityType =
        opportunityType === ANY ||
        item.roles.some((r) => r.opportunity_type === opportunityType);
...
function CompanyRolesCard({
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <article className="rounded-xl border border-border bg-surface p-4 transition-all hover:border-accent/20">
      {/* Header */}
          ...
          {entry.roles.length > 0 && (
            <CollapsibleTrigger asChild>
              <Button variant="ghost" size="sm" className="gap-1.5 text-accent hover:bg-accent/10">
                {isOpen ? "Hide Pipeline" : "View Pipeline"}
                <ChevronDown
