...
function InnovXPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return allEntries.filter((entry) => {
      ...
      const matchesArea =
        areaFilter === ANY || entry.data.innovation_areas.includes(areaFilter);
      ...
      <section className="mt-6">
        {loading ? (
          ...
          <div className="space-y-4">
            {filtered.map((entry) => (
              <InnovXCompanyCard key={entry.id} entry={entry} />
            ))}
          </div>
        )}
      </section>
...
function InnovXCompanyCard({ entry }: { entry: CompanyInnovX }) {
  const { data } = entry;
  const typeColor = TYPE_COLORS[entry.innovationType] ?? "bg-secondary text-muted-foreground border-border";
