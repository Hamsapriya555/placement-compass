...
import { ArrowLeft, ChevronDown, DollarSign, GitCompareArrows, GraduationCap, Lightbulb, ListChecks, Rocket, Sparkles, Target, Zap } from "lucide-react";
...
export const Route = createFileRoute("/company/$companyId")({
  head: () => ({
    meta: [
      { title: "Company Profile · SRM Placement Intelligence" },
      {
        name: "description",
        content:
  ...
  return (
    <div className="mx-auto max-w-screen-2xl px-4 py-8 sm:px-6">
      ...
      <div className="mt-3 flex flex-col items-start gap-4 rounded-2xl border border-border bg-surface p-5 sm:flex-row sm:items-center">
        <CompanyLogoImage
          company={c}
          className="h-[72px] w-[72px] rounded-xl border border-border/50 bg-white object-contain p-1.5 shadow-sm shrink-0"
        />
  ...
  if (error || !data) {
    return (
      <SectionCard
        ...
        description={
          error
            ? "InnovX data could not be loaded."
          ...
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {d.innovation_areas.map((area) => (
                <Badge key={area} variant="outline" className="font-normal">{area}</Badge>
              ))}
            </div>
          </div>
        )}
...
function InnovXProjectCard({ project }: { project: InnovXProject }) {
  const diffColor = DIFF_COLORS[project.difficulty] ?? "bg-secondary text-muted-foreground border-border";
