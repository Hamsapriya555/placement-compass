...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Dashboard · SRM Placement Intelligence" },
      {
        name: "description",
        content:
          "Orientation, discovery and at-a-glance signals across every company hiring on the SRM campus.",
      },
    ],
  }),
  component: HomePage,
    ...
    <div className="relative isolate min-h-screen">
      ...
      <div className="mx-auto max-w-screen-2xl px-4 py-10 sm:px-6 sm:py-16">
        ...
        <section className="relative flex flex-col items-center text-center gap-6">
          <Badge variant="outline" className="px-4 py-1 rounded-full bg-surface border-primary/20 text-primary font-bold tracking-wide shadow-sm animate-in fade-in slide-in-from-top-2 duration-700">
          ...
          <h1 className="font-display text-4xl font-extrabold tracking-tight sm:text-6xl text-balance max-w-4xl text-foreground/90 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            Decide which companies to target — using <span className="text-primary italic">data</span>, not anecdotes.
          </h1>
        ...
        <section className="mt-16 grid grid-cols-2 gap-4 sm:grid-cols-4 animate-in fade-in zoom-in-95 duration-1000 delay-500">
          <StatTile label="Companies tracked" value={total} icon={Building2} className="shadow-sm hover:shadow-md transition-shadow" />
          <StatTile
            label="Categories"
            ...
            {tileCounts.map((t) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-sm font-bold opacity-60 tabular-nums">{t.count}</span>
                </div>
                <div>
                  <div className="font-display text-lg font-bold tracking-tight text-foreground">{t.label}</div>
          ...
          <div className="flex items-center justify-between border-b border-border/60 pb-5">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {q ? `Showing intelligent matches for "${q}"` : "The latest intelligence updates from across the platform."}
              </p>
            </div>
            <Link
              to="/explore"
            ...
            >
              Browse Registry <ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-1" />
            </Link>
          </div>
  ...
  return (
    <div className={cn("rounded-2xl border border-border bg-surface p-6 shadow-sm", className)}>
      <div className="flex items-center justify-between mb-6">
        ...
        <Badge variant="secondary" className="font-bold text-[10px] uppercase tracking-widest px-2">Live View</Badge>
      </div>
        ...
        {loading ? (
          ...
          rows.slice(0, 6).map((r) => (
            <div key={r.label} className="group flex items-center gap-4 text-xs">
              <span className="w-32 font-bold text-foreground/70 group-hover:text-foreground transition-colors truncate">{r.label}</span>
              ...
              <span className="w-10 text-right font-mono text-sm font-bold tabular-nums text-foreground/80">{r.count}</span>
            </div>
          ))
        )}
      </div>
