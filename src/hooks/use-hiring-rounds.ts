...
function extractJobRoles(row: JobRoleRoundRow): JobRole[] {
  ...
  if (roleDetails.length === 0) {
    ...
    if (rounds.length > 0) {
      return [
        {
          role_title: stringFromUnknown(json.role_title ?? json.title ?? json.job_title) ?? "Unknown Role",
  ...
  return roleDetails
    .map((detail) => {
      ...
      return {
        role_title: stringFromUnknown(obj.role_title ?? obj.title ?? obj.job_title) ?? "Unknown Role",
...
function extractRoundsFromRoleObj(
  ...
  return rawRounds.map((round, index) => {
    ...
    return {
      ...
      round_order:
        numberFromUnknown(
          source.round_number ?? source.round_order ?? source.order ?? source.sequence ?? source.round_no,
        ) ?? index + 1,
      ...
      preparation_focus: preparationFocus || (skillSets.length > 0 ? skillSets.map((s) => s.skill_set_code).join(", ") : null),
...
function extractSkillSets(value: unknown): JobRoleSkillSet[] {
  ...
  return arr
    .map((item) => {
      ...
      const questions = stringFromUnknown(obj.typical_questions ?? obj.questions ?? obj.sample_questions);
      if (!code) return null;
...
export function processTags(rounds: HiringRound[]): string[] {
  ...
  if (/interview|hr|behavior|managerial|communication|comm/.test(text)) tags.push("Interview Heavy");
  return tags;
}
