...
const INNOVATION_TAGS: { label: string; pattern: RegExp }[] = [
  { label: "AI-Driven", pattern: /\b(ai|machine learning|ml|deep learning|nlp|gen\s?ai|neural|llm)\b/i },
  { label: "Data-Centric", pattern: /\b(data|analytics|bi|warehouse|lake|etl|pipeline|big data)\b/i },
...
function parseInnovXData(raw: unknown): InnovXData {
  ...
  return {
    company_strategy: str(json.company_strategy),
    digital_transformation_focus: strArr(json.digital_transformation_focus),
    business_problems: strArr(json.business_problems),
    innovation_areas: strArr(json.innovation_areas),
    required_skills: strArr(json.required_skills),
    recommended_projects: parseProjects(json.recommended_projects),
    preparation_strategy: str(json.preparation_strategy),
    differentiation_tip: str(json.differentiation_tip),
  };
}
...
function parseProjects(value: unknown): InnovXProject[] {
  ...
  return arr
    .map((item) => {
      ...
      return {
        title: str(obj.title) ?? "Untitled Project",
        description: str(obj.description) ?? "",
        tech_stack: strArr(obj.tech_stack),
        difficulty: str(obj.difficulty) ?? "Medium",
        expected_outcome: str(obj.expected_outcome) ?? "",
      } satisfies InnovXProject;
    })
    .filter((p) => p.title !== "Untitled Project" || p.description);
}
