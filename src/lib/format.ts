...
export const EMPTY = "Data not available";
...
export function fmtNumber(
  ...
  if (typeof num !== 'number' || Number.isNaN(num)) return EMPTY;
...
export function getLocation(company: any): string {
  if (company.city && company.country) {
    return `${company.city}, ${company.country}`;
  }
  return company.country || company.location || "Location not specified";
}

export function getValue(value: any, fallback: string): string {
