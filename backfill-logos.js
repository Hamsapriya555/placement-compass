...
async function backfillLogos() {
    const { data: companies, error: cError } = await supabase.from('companies').select('company_id, name, website_url');
    const { data: existingLogos, error: lError } = await supabase.from('company_logo').select('company_id');
    ...
    for (const company of missing) {
        if (!company.website_url || company.website_url === 'NA' || company.website_url === 'N/A') continue;
        
        // Extract domain
        let domain = company.website_url.replace(/https?:\/\//, '').replace(/\/.*$/, '').split('?')[0].trim();
